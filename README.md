...
| `mise run t:wh:probe tenant_… [--platform apple\|google\|auto] [--env sandbox\|production]` | End-to-end probe: dispatches a real test event through whichever platform(s) are configured (Apple test-notification + Google Pub/Sub publish) so the backend handler can confirm the chain        |
| `mise run t:wh:ping tenant_… [--format pretty\|json]`                                       | Synchronous delivery test: signs a synthetic webhook and POSTs it straight to the tenant's callback URL, reporting the HTTP status + latency (tests the Attesto→backend leg only; no Apple/Google) |
